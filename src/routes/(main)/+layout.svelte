...
<div
  class="h-screen flex items-start justify-center bg-black text-white overflow-x-hidden overflow-y-auto"
>
  {#if $page.url.pathname !== "/play"}
    <!-- Blurred background -->
    <div
      class="absolute inset-0 bg-center bg-cover bg-fixed blur-lg saturate-90"
      style="background-image: url('/background.png')"
      aria-hidden="true"
    ></div>
    <div
      class="absolute inset-0 bg-gradient-to-b from-black/80 to-black"
      aria-hidden="true"
    ></div>
  {/if}
      ...
      <div class="flex items-center justify-between mb-6">
        ...
        <div class="relative w-1/3">
          ...
          {#if $searchResults.songs.length > 0 || $searchResults.albums.length > 0}
            <div
              class="absolute right-0 left-0 mt-2 bg-black/50 backdrop-blur-md border border-white/10 rounded-xl shadow-lg max-h-72 overflow-auto z-50"
            >
              <!-- Albums -->
              {#if $searchResults.albums.length > 0}
                <div class="p-2 border-b border-white/10">
                  <div
                    class="text-xs text-gray-300 uppercase font-semibold mb-1 px-1"
                  >
                    Albums
                  </div>
                ...
                <div class="p-2">
                  ...
                  {#each $searchResults.songs as s}
                    ...
                    >
                      <div class="flex items-center gap-2 truncate">
                        <img
                          src={s.thumbnail ?? "/default_wallpaper.jpg"}
                          alt="thumb"
                          class="w-8 h-8 rounded object-cover flex-shrink-0"
                        />
                        <div class="truncate">
