...
<script lang="ts">
  ...
  async function handleCreateAlbum() {
    ...
    creatingAlbum = true;
    try {
      const created = (await createAlbum(newAlbumName.trim())) as unknown as {
        id: number;
        name: string;
        createdAt: number;
        thumbnail?: string | null;
...
<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
  ...
  >
    <div class="flex flex-col lg:flex-row gap-6">
      ...
      <div
        class="flex-shrink-0 relative w-full lg:w-64 h-64 rounded-2xl overflow-hidden bg-white/10"
      >
        <img
      ...
      <div class="flex-1 flex flex-col justify-between">
        <div>
          ...
          <div class="mt-3 flex flex-wrap gap-3 items-center">
            ...
            <button
              class="px-4 py-2 bg-indigo-600 rounded-lg text-white font-semibold hover:bg-indigo-500 disabled:opacity-50"
              on:click={handleLoad}
            ...
            <button
              class="px-4 py-2 bg-gray-700/30 rounded-lg text-white hover:bg-gray-700/50 disabled:opacity-50"
              on:click={handleSave}
            ...
            <select
              class="ml-auto bg-white/10 text-gray-500 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-500"
          ...
          <div class="mt-4">
            <div class="text-2xl font-semibold">
              {$info.title ??
                ($info.id ? `Loaded (${String($info.id)})` : "No title")}
            </div>
        ...
        <div class="mt-4">
          <div class="flex items-center gap-4">
            <div class="w-14 text-xs text-gray-400">
            ...
            />
            <div class="w-14 text-right text-xs text-gray-400">
  ...
  <aside
    class="bg-white/5 backdrop-blur-md rounded-2xl p-4 flex flex-col gap-4 shadow-lg"
  >
    ...
    <div class="mt-2">
      <div class="flex gap-2 items-center">
        <input
          ...
          class="flex-1 px-2 py-1 rounded-lg bg-white/10 text-white focus:outline-none focus:ring-2 focus:ring-indigo-500"
        />
        <button
          class="px-3 py-1 bg-indigo-600 rounded-lg text-white hover:bg-indigo-500 disabled:opacity-50"
          on:click={handleCreateAlbum}
          disabled={!newAlbumName || creatingAlbum}
        >
          {#if creatingAlbum}Creating…{:else}Create{/if}
        </button>
      </div>
      <div class="text-xs text-gray-400 mt-2">
        Created albums appear in the album list and in the select above.
      </div>
    </div>
    ...
    <div>
      ...
      <div class="mt-2 flex flex-col gap-2 overflow-y-auto max-h-[40vh]">
        ...
        {#each albums as a}
          <button
            class="w-full text-left flex items-center justify-between p-2 hover:bg-white/10 rounded-lg transition"
            on:click={() => openAlbum(a.id)}
            aria-label={"Open album " + a.name}
          >
            <div class="flex items-center gap-3">
              <div
                class="w-10 h-10 bg-white/10 rounded-lg overflow-hidden flex-shrink-0"
              >
                <img
                  src={a.thumbnail ?? "/default_wallpaper.jpg"}
                  alt="album"
                  class="w-full h-full object-cover"
                />
              </div>
